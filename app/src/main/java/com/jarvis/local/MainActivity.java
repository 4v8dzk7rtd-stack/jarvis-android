...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private static final String CHAT_URL = "http://192.168.1.164:8765/chat";
    ...
    private void buildInterface() {
        ...
        LinearLayout.LayoutParams buttonParams = new LinearLayout.LayoutParams(dp(210), dp(210));
        ...
        conversationView.setText("Нажми кнопку и задай вопрос Джарвису.");
        conversationView.setTextColor(Color.rgb(210, 244, 255));
        conversationView.setTextSize(17);
        conversationView.setPadding(dp(16), dp(16), dp(16), dp(16));
        conversationView.setBackgroundColor(Color.rgb(12, 28, 40));
    ...
    private android.graphics.drawable.GradientDrawable createRoundBackground() {
        android.graphics.drawable.GradientDrawable drawable = new android.graphics.drawable.GradientDrawable();
        drawable.setShape(android.graphics.drawable.GradientDrawable.OVAL);
        drawable.setColor(Color.rgb(7, 79, 106));
        drawable.setStroke(dp(3), Color.rgb(38, 217, 255));
        return drawable;
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SPEECH_REQUEST) {
            if (resultCode == RESULT_OK && data != null) {
                ...
                if (results != null && !results.isEmpty()) sendMessage(results.get(0));
                else statusView.setText("Не расслышал");
            } else {
                statusView.setText("Готов");
            }
        }
    ...
    private void sendMessage(String message) {
        ...
        conversationView.setText("Вы: " + message + "\n\nJARVIS: ...");
        ...
        new Thread(() -> {
            ...
            try {
                ...
                connection.setRequestProperty("X-Jarvis-Token", TOKEN);

                byte[] payload = new JSONObject().put("message", message).toString().getBytes(StandardCharsets.UTF_8);
                ...
                String reply = json.optString("reply", json.optString("error", "Нет ответа"));
                runOnUiThread(() -> showReply(message, reply));
            } catch (Exception e) {
                String error = "Не удалось связаться с компьютером. Проверь Wi‑Fi, сервер и адрес 192.168.1.164:8765.";
    ...
    private void showReply(String message, String reply) {
        conversationView.setText("Вы: " + message + "\n\nJARVIS: " + reply);
        statusView.setText("Отвечаю...");
        microphoneButton.setEnabled(true);
        if (tts != null) {
            tts.speak(reply, TextToSpeech.QUEUE_FLUSH, null, "jarvis_reply");
        }
        statusView.postDelayed(() -> statusView.setText("Готов"), 1000);
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == AUDIO_REQUEST && grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) startListening();
        else Toast.makeText(this, "Без доступа к микрофону голосовой ввод не работает", Toast.LENGTH_LONG).show();
    }
